...
extract_1850 <- ipumsr::submit_extract(
  define_extract_usa(
    samples = "us1850c",
...
ddi_1850 <- ipumsr::wait_for_extract(extract_1850) %>%
  ipumsr::download_extract(download_dir = ipums_data_path)

# Define that data will be extracted from the 1860 full-count US census
extract_1860 <- ipumsr::submit_extract(
  define_extract_usa(
    samples = "us1860c",
...
ddi_1860 <- ipumsr::wait_for_extract(extract_1860) %>%
  ipumsr::download_extract(download_dir = ipums_data_path)
