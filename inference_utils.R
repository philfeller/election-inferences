...
regression_model <- function(results, ei.model, beg_yr, end_yr, selection_method = "all", seed = 123,
  ...
  if (return_coefficients) {
    all_coefs <- map_df(elastic_models, "coefficients")
    return(list(
      selected_covariates = covars,
      coefficients_by_party = all_coefs,
...
build_ei_model <- function(beg_yr, end_yr, lambda1, lambda2, covariate = FALSE, base_model = NULL, cov_comparison = NULL, results_tibble = NULL) {
  # beg_yr: beginning year of election transition
  # end_yr: ending year of election transition
  ...
  # base_model: model object to use as the base model for comparison when adding covariates, typically from a previous run
  # cov_comparison: optional covariate campaison object, typically from a previous run
  ...
  if (covariate == TRUE & is.null(cov_comparison)) {
    ...
    cov_comparison <- as.data.frame(cov_data["coefficients_by_party"]) %>%
      rename(
        direction = coefficients_by_party.direction,
        to_party = coefficients_by_party.to_party,
        covariate = coefficients_by_party.covariate
      ) %>%
      select(covariate, to_party, direction) %>%
      mutate(
        to_party = gsub("_", " ", gsub(paste0("_in_", end_yr), "", to_party)),
  ...
  return(list(model = ei.model, base_model = base.ei.model, cov = cov_comparison))
}
