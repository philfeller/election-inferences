...
setwd(dirname(wd))
```
I examine uncertainty in the model using three complementary approaches. One, illustrated in the leftmost heatmaps below, looks at the percentage of the variation in the model's estimates that is due to runs beginning with a different random starting point. This is known as "between-run" variability. A second way, shown in the middle heatmaps below, looks at the percentage of pairwise run comparisons that have non-overlapping 95% credible intervals. The third way, shown in the rightmost heatmaps, looks at the coefficient of variation (CV) for each transition, which is the standard deviation of the estimates divided by the mean estimate.
